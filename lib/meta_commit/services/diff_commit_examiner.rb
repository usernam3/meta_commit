...
module MetaCommit::Services
  class DiffCommitExaminer
    ...
    # region meta
    def meta(left_commit, right_commit)
      changes = MetaCommit::Models::Changes::Repository.new(@repo.repo.path)
      diff = @repo.diff(left_commit, right_commit, DIFF_OPTIONS)
      changes.push(examine_commits_diff(diff, left_commit, right_commit))
    end
