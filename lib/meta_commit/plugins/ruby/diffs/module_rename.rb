module MetaCommit::Plugin::Ruby::Diffs
  class ModuleRename < Diff
    def supports_change(type, old_file_name, new_file_name, old_ast_path, new_ast_path)
      type == MetaCommit::Plugin::Ruby::Diffs::Diff::TYPE_REPLACE && !old_ast_path.target_node.empty_ast? && !new_ast_path.target_node.empty_ast? && is_name_of_module?(old_ast_path) && is_name_of_module?(new_ast_path)
    end
